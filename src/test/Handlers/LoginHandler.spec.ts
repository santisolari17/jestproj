...
describe.skip('LoginHandler test suite', () => {
  ...
  test('post request with valid login', async () => {
    ...
    getRequestBodyMock.mockReturnValueOnce({ username: 'someuser', password: 'password'})
  ...
  test('post request with unexpected error', async () => {
    requestMock.method = HTTP_METHODS.POST;
    getRequestBodyMock.mockRejectedValueOnce(new Error('some unexpected error message'))

    await loginHandler.handleRequest();

    expect(responseMock.statusCode).toBe(HTTP_CODES.INTERNAL_SERVER_ERROR);
    expect(responseMock.write).toHaveBeenCalledWith('Internal error: some unexpected error message');
  })
});
